...
//	 Свои загрузчики формы из xml
//////////////////////////////////////////////////////////////////////////////////

#Область ПрограммныйИнтерфейс

#Область СозданиеПоXML
// Процедура - Собрать группу элементов из XML
//
// Параметры:
//  Форма					 - ФормаКлиентскогоПриложения - Форма на которой создаются элементы
//  ОбъектДляПостройки			     - ОбъектXDTO,текст - Объект в который призвели чтение XML фабрикой xdto для сборки, см. 
//  ЭлементРодитель			 - Строка - Имя элемента на форме, если параметр не указан, родителем считается форма 
//  ДополнительныеПараметры	 Структура - Дополнительные параметры, см. ПолучитьДоппареметрыДляСбораФормы() этого модуля 
//
Процедура СобратьГруппуЭлементовИзXML(Форма,ОбъектДляПостройки,ЭлементРодитель = Неопределено, ДополнительныеПараметры = Неопределено)  Экспорт
	
	
	СтруктураЭлементов = pr_ДинамическоеФормированиеИнтерфейсаПовтИсп.СтруктураЗагружаемыхЭлементовФормы();
	Если ДополнительныеПараметры = Неопределено Тогда
		ДополнительныеПараметры = ПолучитьДоппараметрыДляСбораФормы();
	КонецЕсли;
	СтроитьРеквизиты = ДополнительныеПараметры.СтроитьРеквизиты;
	СтроитьКоманды   = ДополнительныеПараметры.СтроитьКоманды;
	СтроитьПодчиненныеЭлементы  = ДополнительныеПараметры.СтроитьПодчиненныеЭлементы;
	
	Если ТИПЗНЧ(ОбъектДляПостройки) = ТИП("ОбъектXDTO") Тогда
		
		СтроимИз = ОбъектДляПостройки;
		
	ИначеЕсли ТИПЗНЧ(ОбъектДляПостройки) = ТИП("Строка") тогда
		
		Читаем = Новый ЧтениеXML;
		Читаем.УстановитьСтроку(ОбъектДляПостройки);
		СтроимИз = ФабрикаXDTO.ПрочитатьXML(Читаем);	
	...
	СвойстваОбъекта = СтроимИз.Свойства();
	
	#Область Построение_реквизитов
	Реквизиты = СвойстваОбъекта.Получить("Attributes");
	Если Реквизиты <> неопределено и Строитьреквизиты Тогда
		
		ПостроитьРеквизитыИзОбъектаXDTO(Форма,СтроимИз.Attributes); 
		
	КонецЕсли;
	#КонецОбласти
	
	#Область Построение_КомандФормы
	
	Команды = СвойстваОбъекта.Получить("Commands");
	
	Если Команды <> неопределено и СтроитьКоманды Тогда
		
		ПостроитьКомандыИзОбъектаXDTO(Форма,СтроимИз.Commands); 
	...
	#Область Построение_подчиненные_элементы_формы
	
	ПодчиненныеЭлементы = СвойстваОбъекта.Получить("ChildItems");
	
	Если ПодчиненныеЭлементы <> Неопределено и СтроитьПодчиненныеЭлементы Тогда
		
		ПостроитьПодчиненныеЭлементыИзОбъектаXDTO(Форма,СтроимИз.ChildItems,ЭлементРодитель); 
...
Функция ОпределитьТипЭлементаФормы(ИмяЭлемента)
	
	Если ИмяЭлемента = "UsualGroup"
		ИЛи ИмяЭлемента = "CommandBar"
		Тогда
		
		ТипЭлементаФормы = Тип("ГруппаФормы");
	ИначеЕсли ИмяЭлемента = "Button" Тогда 
		
		ТипЭлементаФормы = Тип("КнопкаФормы");
		
	ИначеЕсли ИмяЭлемента = "PictureField"  Тогда
		
		ТипЭлементаФормы = Тип("ПолеФормы");
		
	ИначеЕсли  ИмяЭлемента = "LabelDecoration"  Тогда
		
		ТипЭлементаФормы = Тип("ДекорацияФормы");

	КонецЕсли;
	
	Возврат ТипЭлементаФормы;
	
КонецФункции

Функция ПолучитьДоппараметрыДляСбораФормы() Экспорт
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("СтроитьРеквизиты",Истина);
	СтруктураПараметров.Вставить("СтроитьКоманды",Истина);
	СтруктураПараметров.Вставить("СтроитьПодчиненныеЭлементы",Истина);
	
	
	Возврат СтруктураПараметров;
КонецФункции

Процедура ПостроитьРеквизитыИзОбъектаXDTO(Форма,ОбъектСОписаниемРеквизитов)
	СтрРеквизитов = Новый Структура;
	
	СвойстваОбъектаСРеквизитом = ОбъектСОписаниемРеквизитов.Свойства();
	Если СвойстваОбъектаСРеквизитом.Получить("Attribute")<> Неопределено Тогда
		Если ТИПЗНЧ(ОбъектСОписаниемРеквизитов.Attribute) = ТИП("СписокXDTO") Тогда
			Для каждого ЭлементСписка Из ОбъектСОписаниемРеквизитов.Attribute Цикл
				Если ЭлементСписка.name = "Объект" Тогда
					Продолжить;
				Иначе
					ИмяРеквизита = ЭлементСписка.name;
					ТипРеквизита = ВернутьОписаниеТипаПоОбъекту(ЭлементСписка.Type);
					СтрРеквизитов.Вставить(ИмяРеквизита,Типреквизита); 
	...
	pr_ДинамическоеФормированиеИнтерфейсаСервер.СоздатьРеквизиты(Форма,СтрРеквизитов);
	
КонецПроцедуры

Процедура ПостроитьКомандыИзОбъектаXDTO(Форма,ОбъектСОписаниемКоманд)
	
	СтруктураЭлементов = pr_ДинамическоеФормированиеИнтерфейсаПовтИсп.СтруктураЗагружаемыхЭлементовФормы();
	
	СвойстваОбъектаСКомандами = ОбъектСОписаниемКоманд.Свойства();
	Если СвойстваОбъектаСКомандами.Получить("Command")<> Неопределено Тогда
		Если ТИПЗНЧ(ОбъектСОписаниемКоманд.Command) = ТИП("СписокXDTO") Тогда
			Для каждого ЭлементСписка Из ОбъектСОписаниемКоманд.Command Цикл
				
				ДобавитьКомандуИзОбъектаXDTO(Форма,ЭлементСписка,СтруктураЭлементов.Command);
...
Функция ТипыОбрабоатываемВручнуюПока()
	МассивПропускаемыхСвойств = Новый Массив;
	МассивПропускаемыхСвойств.Добавить("name");
	МассивПропускаемыхСвойств.Добавить("id");
	МассивПропускаемыхСвойств.Добавить("ChildItems");
	МассивПропускаемыхСвойств.Добавить("Title");
	МассивПропускаемыхСвойств.Добавить("ToolTip");
	МассивПропускаемыхСвойств.Добавить("ExtendedTooltip");
	МассивПропускаемыхСвойств.Добавить("Representation");
	МассивПропускаемыхСвойств.Добавить("ChildItemsWidth");
	МассивПропускаемыхСвойств.Добавить("ShowTitle");
	МассивПропускаемыхСвойств.Добавить("HorizontalStretch"); //Растягивать по горизонтали
	МассивПропускаемыхСвойств.Добавить("TitleFont"); //Шрифт заголовка
	МассивПропускаемыхСвойств.Добавить("Type"); 
	МассивПропускаемыхСвойств.Добавить("CommandName");
	МассивПропускаемыхСвойств.Добавить("TitleLocation");
	МассивПропускаемыхСвойств.Добавить("VerticalStretch");
	МассивПропускаемыхСвойств.Добавить("PictureSize");
	МассивПропускаемыхСвойств.Добавить("FileDragMode");
	МассивПропускаемыхСвойств.Добавить("ContextMenu");
	МассивПропускаемыхСвойств.Добавить("Event");
	//МассивПропускаемыхСвойств.Добавить("Hyperlink");
	МассивПропускаемыхСвойств.Добавить("NonselectedPictureText");
	МассивПропускаемыхСвойств.Добавить("TextColor");
	МассивПропускаемыхСвойств.Добавить("Events");
	МассивПропускаемыхСвойств.Добавить("Font");
	МассивПропускаемыхСвойств.Добавить("ToolTipRepresentation");
	Возврат МассивПропускаемыхСвойств;
КонецФункции

Процедура ПостроитьПодчиненныеЭлементы(Форма,КорневойЭлементXDTO,Родитель,ИмяТипаЭлемента,СтруктураЭлементов)
	
	МассивПропускаемыхСвойств = ТипыОбрабоатываемВручнуюПока();
	
	СвойстваЭлементов = СтруктураЭлементов.Properties;
	ЗначенияСвойствЭлементов = СтруктураЭлементов.ЗначенияСвойств; 
	СвойстваОбъектаСЭлементами = КорневойЭлементXDTO.Свойства();
	
	ИмяКорневогоЭлемента       = КорневойЭлементXDTO.name;
	
	КорневойЭлемент = Форма.Элементы.Найти(КорневойЭлементXDTO.name);
	
	Если КорневойЭлемент = Неопределено Тогда
		
		ТипЭлементаФормы = ОпределитьТипЭлементаФормы(ИмяТипаЭлемента);
		
		КорневойЭлемент = Форма.Элементы.Добавить(КорневойЭлементXDTO.name, 
		
		ТипЭлементаФормы,Родитель);
		//
		Если ИмяТипаЭлемента = "UsualGroup" Тогда
			
			КорневойЭлемент.Вид = ВидГруппыФормы.ОбычнаяГруппа; 
			
		ИначеЕсли ИмяТипаЭлемента = "CommandBar"  Тогда
			
			КорневойЭлемент.Вид = ВидГруппыФормы.КоманднаяПанель; 
		ИначеЕсли ИмяТипаЭлемента = "Button" Тогда
			
			Если КорневойЭлементXDTO.Свойства().Получить("type") <> Неопределено Тогда
				
				ЗначенияСвойства =  ЗначенияСвойствЭлементов["ТипыКнопок"];
				
				КорневойЭлемент.Вид = ЗначенияСвойства[КорневойЭлементXDTO["type"]];
			Иначе
				КорневойЭлемент.Вид = ВидКнопкиФормы.ОбычнаяКнопка;
				
			КонецЕсли;
			
			Если КорневойЭлементXDTO.Свойства().Получить("CommandName") <> Неопределено Тогда
				
				КорневойЭлемент.ИмяКоманды = СтрЗаменить(КорневойЭлементXDTO["CommandName"],"Form.Command.","");
				
			КонецЕсли;
		ИначеЕсли ИмяТипаЭлемента = "PictureField" Тогда
			
			КорневойЭлемент.Вид  = ВидПоляФормы.ПолеКартинки;
			
		иначеЕсли	ИмяТипаЭлемента = "LabelDecoration" Тогда
			
			А = 0;//КорневойЭлемент.Вид  = ВидПоляФормы.ПолеКартинки;

		КонецЕсли;	
		
		Для каждого СвойствоКорневого Из КорневойЭлементXDTO.Свойства() Цикл
			
			Если МассивПропускаемыхСвойств.Найти(СвойствоКорневого.Имя)=Неопределено Тогда
				
				КлючЭлемента =  СвойстваЭлементов[СвойствоКорневого.Имя];
				
				ЗначенияСвойства =  ЗначенияСвойствЭлементов[СвойствоКорневого.Имя];
				
				Если ЗначенияСвойства <> неопределено Тогда
					Если ТИПЗНЧ(ЗначенияСвойства) = ТИП("Тип") Тогда   // Если тип Тогда по значению находим
						Значение =  XMLЗначение(ЗначенияСвойства,КорневойЭлементXDTO[СвойствоКорневого.Имя]);
					ИначеЕсли  ТИПЗНЧ(ЗначенияСвойства) = ТИП("СписокXDTO")  Тогда
						Значение =  ОбработатьСвойствоЭлементаПоСпискуXDTO(ЗначенияСвойства);
					Иначе
						Значение = ЗначенияСвойства[КорневойЭлементXDTO[СвойствоКорневого.Имя]];
					КонецЕсли;
					
					КорневойЭлемент[КлючЭлемента] = Значение;
					
				Иначе
					
					КорневойЭлемент[КлючЭлемента] = КорневойЭлементXDTO[СвойствоКорневого.Имя];
		...
		//Строим вложенные элементы если есть	
		ЕстьПодчиненные = КорневойЭлементXDTO.Свойства().Получить("ChildItems");
		
		Если ЕстьПодчиненные <> Неопределено Тогда
			
			СвойстваПодчиненых = КорневойЭлементXDTO.ChildItems.Свойства();
			
			Для каждого Свойство Из СвойстваПодчиненых Цикл
				
				ГруппаПодчиненыхЭлементов =   КорневойЭлементXDTO.ChildItems[Свойство.Имя];
				
				Если ТИПЗНЧ(ГруппаПодчиненыхЭлементов) = ТИП("СписокXDTO") Тогда
					
					Для каждого ЭлементСписка Из ГруппаПодчиненыхЭлементов Цикл
						
						ПостроитьПодчиненныеЭлементы(Форма,ЭлементСписка,КорневойЭлемент,Свойство.Имя,СтруктураЭлементов);
						
					КонецЦикла;
					
				ИначеЕсли ТИПЗНЧ(ГруппаПодчиненыхЭлементов) = ТИП("ОбъектXDTO") Тогда
					
					ПостроитьПодчиненныеЭлементы(Форма,ГруппаПодчиненыхЭлементов,КорневойЭлемент,Свойство.Имя,СтруктураЭлементов);
...
Процедура ПостроитьПодчиненныеЭлементыИзОбъектаXDTO(Форма,ОбъектСОписаниемЭлементов,ЭлементРодитель)
	
	МассивПропускаемыхСвойств = ТипыОбрабоатываемВручнуюПока();
	
	СтруктураЭлементов = pr_ДинамическоеФормированиеИнтерфейсаПовтИсп.СтруктураЗагружаемыхЭлементовФормы().ChildItems;
	СвойстваЭлементов = СтруктураЭлементов.Properties;
	ЗначенияСвойствЭлементов = СтруктураЭлементов.ЗначенияСвойств; 
	//Предполагаем, что будем строить всегда одну ветку
	СвойстваОбъектаСЭлементами = ОбъектСОписаниемЭлементов.Свойства();
	ИмяКорневогоЭлемента       = СвойстваОбъектаСЭлементами[0].Имя;
	
	Если СвойстваОбъектаСЭлементами.Получить(ИмяКорневогоЭлемента)<> Неопределено Тогда
		
		КорневойЭлементXDTO = ОбъектСОписаниемЭлементов[ИмяКорневогоЭлемента];
		КорневойЭлемент = Форма.Элементы.Найти(КорневойЭлементXDTO.name);
		Если КорневойЭлемент = Неопределено Тогда
			Если ЭлементРодитель = "" Тогда
				Родитель = Форма;
			Иначе
				Родитель = Форма.Элементы.Найти(ЭлементРодитель);

			КонецЕсли;
			//Родитель
			ТипЭлементаФормы = ОпределитьТипЭлементаФормы(ИмяКорневогоЭлемента);
			КорневойЭлемент = Форма.Элементы.Добавить(КорневойЭлементXDTO.name, 
			ТипЭлементаФормы,Родитель);
			
			Если ИмяКорневогоЭлемента = "UsualGroup" Тогда
				
				КорневойЭлемент.Вид = ВидГруппыФормы.ОбычнаяГруппа;
				
			ИначеЕсли ИмяКорневогоЭлемента = "CommandBar"  Тогда
				
				КорневойЭлемент.Вид = ВидГруппыФормы.КоманднаяПанель; 
				
			ИначеЕсли ИмяКорневогоЭлемента = "Button" Тогда
				...
				Если КорневойЭлементXDTO.Свойства().Получить("CommandName") <> Неопределено Тогда
					...
					КорневойЭлемент.Вид = СтрЗаменить(КорневойЭлементXDTO["CommandName"],"Form.Command.","");
					
				КонецЕсли;
			ИначеЕсли ИмяКорневогоЭлемента = "PictureField" Тогда
			...
			Для каждого СвойствоКорневого Из КорневойЭлементXDTO.Свойства() Цикл
				...
				Если МассивПропускаемыхСвойств.Найти(СвойствоКорневого.Имя)=Неопределено Тогда
					...
					КорневойЭлемент[КлючЭлемента] = ЗначенияСвойства[КорневойЭлементXDTO[СвойствоКорневого.Имя]];
	...
	//
	//ГруппаПодписьФаксимиле.Вид        = ВидГруппыФормы.ОбычнаяГруппа;
	//ГруппаПодписьФаксимиле.Заголовок  = "Факсимиле";
	//ГруппаПодписьФаксимиле.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	//ГруппаПодписьФаксимиле.Ширина = 42;
	
	//
	//
	//ГруппаКоманднаяПанельФаксимиле =  Форма.Элементы.Добавить("ГруппаКоманднаяПанельФаксимиле", 
	//Тип("ГруппаФормы"),ГруппаПодписьФаксимиле);
	//
	//ГруппаКоманднаяПанельФаксимиле.Вид = ВидГруппыФормы.КоманднаяПанель;
	//
	//// Команда ДобавитьИзображениеФаксимиле
	//
	//СтруктураПараметров = ПолучитьСтруктуруПараметровСозданияКнопкиСКомандой();
	//СтруктураПараметров.ИмяКоманды        = "КомандаДобавитьИзображениеФаксимиле";   
	//СтруктураПараметров.ОбработчикКоманды = "Подключаемый_ВыполнитьПереопределяемуюКоманду";   
	//СтруктураПараметров.ИмяКнопкиКоманды ="ФормаДобавитьИзображениеФаксимиле";
	//СтруктураПараметров.ИмяГруппыКнопки  = "ГруппаКоманднаяПанельФаксимиле"; //Имя группы на форме, куда вставляем команду
	//СтруктураПараметров.ЗаголовокКнопки = " ";    // Будет видно пользователю как имя кнопки
	//ДобавитьКнопкуСКомандойНаФорму(Форма,СтруктураПараметров);
	//КнопкаДобавить = Форма.Элементы.Найти("ФормаДобавитьИзображениеФаксимиле");
	//КнопкаДобавить.Картинка =  БиблиотекаКартинок.СоздатьЭлементСписка;
	//
	// // поле картинки
	// 
	//ПолеКартинкиФаксимиле =  Форма.Элементы.Добавить("АдресФаксимильнойПечати", 
	//Тип("ПолеФормы"),ГруппаПодписьФаксимиле);
	//ПолеКартинкиФаксимиле.Вид = ВидПоляФормы.ПолеКартинки;
	//ПолеКартинкиФаксимиле.ПутьКДанным = "АдресФаксимильнойПечати";
	//ПолеКартинкиФаксимиле.ТекстНевыбраннойКартинки = "Добавить факсимиле";
	//ПолеКартинкиФаксимиле.Ширина = 41;
	//ПолеКартинкиФаксимиле.Высота = 7;
	
КонецПроцедуры

Процедура ДобавитьКомандуИзОбъектаXDTO(Форма,ЭлементСписка,СвойстваКоманд)
	СоответствиеСвойства =  СвойстваКоманд.Properties;    // структура
	ЗначенияСвойств = СвойстваКоманд.ЗначенияСвойств;
	
	НоваяКоманда = Форма.Команды.Добавить(ЭлементСписка.name);
	
	Для каждого Свойство Из ЭлементСписка.Свойства() Цикл
		Если Свойство.Имя = "name" или Свойство.Имя = "id"
			//или Свойство.Имя = "Title"
			или Свойство.Имя = "ToolTip"
			
			Тогда
			Продолжить;
		КонецЕсли;
		
		ДоступныеЗначения = ЗначенияСвойств[Свойство.Имя];			  
		Если ДоступныеЗначения <> Неопределено  Тогда
			
			Если ТИПЗНЧ(ЭлементСписка[Свойство.Имя])= ТИП("ОбъектXDTO") Тогда
				
				Если Свойство.Имя = "Picture" Тогда
					ОбрабатываемыйПодчиненный = ЭлементСписка[Свойство.Имя];
					
					ПутьКартинки =     ОбрабатываемыйПодчиненный["ref"];
					
					Если ПутьКартинки <> Неопределено Тогда
						НоваяКоманда[Свойство.Имя] = ДоступныеЗначения["ref"][ПутьКартинки];
					КонецЕсли;
					
				ИначеЕсли  Свойство.Имя = "Title"  Тогда
					Если  ЭлементСписка[Свойство.Имя].Свойства().получить("item")<>Неопределено    Тогда
						Значение = ОбработатьСвойствоЭлементаПоСпискуXDTO(ЭлементСписка[Свойство.Имя].item);
						НоваяКоманда[Свойство.Имя] = Значение;
			...
			Иначе
				...
				Значение = ЭлементСписка[Свойство.Имя];
				
				
				Если ТИПЗНЧ(ДоступныеЗначения) = ТИП("Тип") Тогда   // Если тип Тогда по значению находим
					
					НоваяКоманда[СоответствиеСвойства[Свойство.Имя]] = XMLЗначение(ДоступныеЗначения,Значение);
					
				ИначеЕсли  ТИПЗНЧ(ДоступныеЗначения) = ТИП("СписокXDTO")  Тогда
					//Значение =  ОбработатьСвойствоЭлементаПоСпискуXDTO(ЗначенияСвойства);
				Иначе
					НоваяКоманда[СоответствиеСвойства[Свойство.Имя]] = ДоступныеЗначения[Значение];
				КонецЕсли;
				
			КонецЕсли;
		Иначе
			
			НоваяКоманда[СоответствиеСвойства[Свойство.Имя]] = ЭлементСписка[Свойство.Имя];
...
Функция ВернутьОписаниеТипаПоОбъекту(ТипДляразбора)
	
	ОписаниеТипа = Неопределено;
	
	Если ТипЗНЧ(ТипДляразбора) = ТИП("ОбъектXDTO") Тогда
		
		Если ТипДляразбора.Type = "xs:string" Тогда
			
			Если ТипДляразбора.свойства().получить("StringQualifiers")<>неопределено Тогда
				
				Квалификатор = ТипДляразбора.StringQualifiers;
				
				Длина = ТипДляразбора.StringQualifiers.Length;
				
				ОписаниеТипа = pr_ДинамическоеФормированиеИнтерфейсаСервер.ОписаниеТипаСтрока(Длина);
				
			КонецЕсли;
			
		ИначеЕсли ТипДляразбора.Type = "xs:boolean" Тогда	
			
			ОписаниеТипа = pr_ДинамическоеФормированиеИнтерфейсаСервер.ОписаниеТипаБулево();
...
Функция ОбработатьСвойствоЭлементаПоСпискуXDTO(ЗначенияСвойства)
	
	Если ТИПЗНЧ(ЗначенияСвойства) = ТИП("ОбъектXDTO") Тогда
		
		МассивЗначений = Новый Массив;
		МассивЗначений.Добавить(ЗначенияСвойства);
		
	иначе
		
		МассивЗначений = ЗначенияСвойства;
	...
	//Обрабатываем item
	//Если пр_ОбщегоНазначения.ЕстьОбщийМодуль("ОбщегоНазначения") Тогда
	...
	Для каждого Элемент ИЗ МассивЗначений Цикл
		Если Элемент.lang = КодЯзыка Тогда
			Ответ = Элемент.content;
		КонецЕсли
	КонецЦикла;
